...
impl LivingEntity for LlamaEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_LLAMA_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_LLAMA_DEATH) }
...
impl Mob for LlamaEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_LLAMA_AMBIENT) }
