//! Vanilla Fox entity implementation.
...
impl LivingEntity for FoxEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_FOX_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_FOX_DEATH) }
...
impl Mob for FoxEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_FOX_AMBIENT) }
