//! Vanilla Parrot entity implementation.
...
impl LivingEntity for ParrotEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_PARROT_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_PARROT_DEATH) }
...
impl Mob for ParrotEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_PARROT_AMBIENT) }
