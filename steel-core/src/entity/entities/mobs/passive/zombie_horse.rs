...
impl LivingEntity for ZombieHorseEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_ZOMBIE_HORSE_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_ZOMBIE_HORSE_DEATH) }
...
impl Mob for ZombieHorseEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_ZOMBIE_HORSE_AMBIENT) }
