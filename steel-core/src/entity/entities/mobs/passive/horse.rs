//! Vanilla Horse entity implementation.
...
impl HorseEntity {
    ...
    /// Returns the horse's current temper.
    #[must_use]
    pub fn temper(&self) -> i32 {
        *self.temper.lock()
    }

    /// Sets the horse's temper.
    ...
    /// Returns the horse's variant integer.
    #[must_use]
    pub fn variant(&self) -> i32 {
        *self.entity_data.lock().id_type_variant.get()
    }

    /// Sets the horse's variant integer.
    pub fn set_variant(&self, variant: i32) {
        self.entity_data.lock().id_type_variant.set(variant);
    }
    ...
    fn handle_eating_and_temper_item(&self, player: &Player, item_stack: &ItemStack) -> bool {
        ...
        if healed {
            self.play_sound(&sound_events::ENTITY_HORSE_EAT, 1.0, 1.0);
            return true;
        }
...
impl LivingEntity for HorseEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_HORSE_HURT)
    }

    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_HORSE_DEATH)
    }
    ...
    fn equip_sound(&self, slot: EquipmentSlot, _stack: &ItemStack) -> Option<SoundEventRef> {
        match slot {
            ...
            EquipmentSlot::Body => Some(&sound_events::ENTITY_HORSE_ARMOR),
            _ => None,
        }
    }
...
impl Mob for HorseEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_HORSE_AMBIENT)
    }
