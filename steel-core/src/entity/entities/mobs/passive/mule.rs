...
impl MuleEntity {
    ...
    /// Returns whether the mule has a chest equipped.
    #[must_use]
    ...
    /// Sets whether the mule has a chest equipped.
    ...
    fn handle_eating_and_temper_item(&self, player: &Player, item_stack: &ItemStack) -> bool {
        ...
        if healed {
            self.play_sound(&sound_events::ENTITY_MULE_EAT, 1.0, 1.0);
            return true;
        }
...
impl LivingEntity for MuleEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_MULE_HURT)
    }

    fn death_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_MULE_DEATH)
    }
...
impl Mob for MuleEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> {
        Some(&sound_events::ENTITY_MULE_AMBIENT)
    }
    ...
    fn mob_interact(&self, player: &Player, hand: InteractionHand) -> InteractionResult {
        ...
        if !item_stack.is_empty() {
                ...
                && !AgeableMob::is_baby(self)
            {
                ...
                self.play_sound(&sound_events::ENTITY_MULE_CHEST, 1.0, 1.0);
                if !player.abilities.lock().instabuild {
                    let mut inventory = player.inventory.lock();
                    let mut current_stack = inventory.get_item_in_hand(hand).clone();
                    current_stack.shrink(1);
