//! Vanilla Drowned entity implementation.
...
impl LivingEntity for DrownedEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_DROWNED_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_DROWNED_DEATH) }
...
impl Mob for DrownedEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_DROWNED_AMBIENT) }
