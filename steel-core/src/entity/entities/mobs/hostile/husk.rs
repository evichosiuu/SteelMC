//! Vanilla Husk entity implementation.
...
impl LivingEntity for HuskEntity {
    ...
    fn hurt_sound(&self, _source: &DamageSource) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_HUSK_HURT) }
    fn death_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_HUSK_DEATH) }
...
impl Mob for HuskEntity {
    ...
    fn ambient_sound(&self) -> Option<SoundEventRef> { Some(&sound_events::ENTITY_HUSK_AMBIENT) }
