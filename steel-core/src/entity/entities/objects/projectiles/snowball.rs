//! Thrown snowball entity (`Snowball`).
//!
//! Mirrors vanilla `Snowball` on the Steel
//! `Projectile → ThrowableProjectile → ThrowableItemProjectile` trait stack.
//! On collision with an entity, it deals 3.0 damage if the entity is a Blaze,
//! or 0.0 damage otherwise, with damage type `vanilla_damage_types::THROWN`.
...
/// Damage dealt to Blazes when hit by a snowball (vanilla `Snowball.onHitEntity`).
const BLAZE_DAMAGE: f32 = 3.0;

/// A thrown snowball projectile.
...
impl SnowballEntity {
    /// Creates a new snowball with no owner and default rendered item.
    #[must_use]
...
impl Projectile for SnowballEntity {
    ...
    fn on_hit(&self, hit: &ProjectileHit) {
        ...
        // VANILLA CLIENT-LOCAL: Snowball particles/events on impact.
...
mod tests {
    ...
    use super::SnowballEntity;

    #[test]
    fn default_item_is_snowball() {
        ...
        assert_eq!(snowball.get_default_item().key, vanilla_items::SNOWBALL.key);
    }
}
