...
fn insertion_rejects_an_entity_owned_by_another_chunk() {
    ...
    assert!(
        chunk
            .set_block_state(local_pos, stone, UpdateFlags::UPDATE_NONE)
            .is_some()
    );
...
fn stale_no_entity_promotion_cannot_consume_a_replacement_marker() {
    ...
    let jukebox = vanilla_blocks::JUKEBOX.default_state();
    assert_eq!(
        chunk.set_block_state(pos, jukebox, UpdateFlags::UPDATE_NONE),
        Some(moving_piston)
    );
